...
class notesController extends Controller
{
    ...
    public function note_ini_edit(Request $request,$m_id,$p_id,$n_id){

        $patient = patient::find($p_id);
        $medico = medico::find($m_id);
        $note = note::find($n_id);
        //
        $vital_sign_config = vital_sign::where('note_id',$note->id)->first();
        //
        if($request->expedient_id == Null){
            $expedient = Null;
        }else{
        ...
        $vital_signs = vital_sign::where('note_id',$note->id)->get();
        // dd($vital_signs);
        return view('medico.notes.note_ini_edit',compact('patient','medico','note','expedient','vital_signs','test_labs'));
    }
    ...
    public function note_store(Request $request){
        ...
        // $test_labs = test_lab::all();
        //
        // foreach ($test_labs as $tl) {
        //
        //     $tl->answer = $request[$tl->question];
        //     $tl->save();
        // }
        //
        // return back()->with('success', 'value');
        // $test_urea = test_lab::where('question', 'Urea')->first();
        //     $test_lab->answer = $request->Urea;
        //     $test_lab->save();
